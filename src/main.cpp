...
String text_print_status = "standby"; // 출력 상태
String text_print_file_name = "No Printfile";  // 출력 파일 이름
...
static lv_disp_draw_buf_t draw_buf;    // 디스플레이 변수 설정
static lv_color_t buf[TFT_WIDTH*10]; // 캐시 새로 고침 설정
...
void init_label_print_status()
{
    ...
    lv_obj_align(label_print_status, LV_ALIGN_CENTER, 0, 50); // 가운데 정렬 표시
}
...
void init_arc_print_progress()
{
    ...
    lv_obj_set_style_arc_color(arc_print_progress, lv_color_hex(0x00E0FF), LV_PART_INDICATOR);// 스피너 색상 
    lv_obj_set_size(arc_print_progress,240,240);                   // 스피너 출력 화면 크기 설정
    lv_arc_set_rotation(arc_print_progress,270);                   // 스피너 영점 조절
    lv_arc_set_bg_angles(arc_print_progress,0,360);                // 스피너 각도 설정
    lv_arc_set_value(arc_print_progress,0);                        // 스피너 시작 값 설정
    lv_obj_center(arc_print_progress);                             // 가운데 정렬 후 표시
}
...
void update_arc_print_progress(){
  ...
  lv_obj_clear_flag(arc_print_progress, LV_OBJ_FLAG_CLICKABLE);
  lv_obj_set_style_arc_color(arc_print_progress, lv_color_hex(0x00E0FF), LV_PART_INDICATOR); // ff0000/进度条颜色
      ...
      if ((WiFi.status() == WL_CONNECTED) && (KeyDownFlag != KEY_DWON) && (start_http_request_flg == 1)) {    //wifi已经连接成功，发送http请求获取数据
          ...
          if (httpCode > 0) { //Check for the returning code
              ...
              if(httpswitch==1){  //喷头热床温度显示
                  ...
                  httpswitch = 3;
              }else if(httpswitch == 3){   // G28 호밍 상태 표시
                  ...
                  httpswitch = 4;
              }else if(httpswitch == 4){   // QGL 상태 표시
                  ...
                  httpswitch = 5;
              }else if(httpswitch == 5){   // Calibrate_z 상태 표시
